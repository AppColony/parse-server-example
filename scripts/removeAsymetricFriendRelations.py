...
parser = argparse.ArgumentParser(description='Remove asymetric friend relations for all Users on OneTap. Defaults to dev, pass the correct keys to use another app.')
...
while len(friend_relations) > 0:
    for friend_relation in friend_relations:
        ...
        inverse_friend_relation_hash = friend_relation.friendUser.objectId + " -> " + friend_relation.user.objectId
        if inverse_friend_relation_hash in friend_relation_by_hash:
            del friend_relation_by_hash[inverse_friend_relation_hash]
        else:
            friend_relation_by_hash[friend_relation_hash] = friend_relation

    page_number += 1
    friend_relations = POFriendRelation.Query.all().order_by("createdAt").limit(1000).skip(page_number * 1000)

for friend_relation in friend_relation_by_hash.itervalues():
